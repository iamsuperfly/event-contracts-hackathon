...
export function formatSettingsHelp(system: SystemRiskLimits): string {
  return [
    ...
    "",
    "/settings — show your current limits",
    "/settings stake <amount> — default amount used for each trade",
    "/settings max stake <amount> — maximum amount used for one trade",
    "/settings max daily loss <amount> — maximum loss allowed per day",
    "/settings max positions <count> — maximum active trades at once",
    "/settings profit target <amount|off> — pause new trades after this daily profit",
    "/settings trading on|off — enable or disable trading",
    "/settings mode paper|testnet — paper never sends on-chain orders",
    "",
    ...
    "Values above these limits are rejected (not silently reduced).",
    "",
    `Starting defaults: stake ${DEFAULT_USER_PREFERENCES.defaultStake}, max stake ${DEFAULT_USER_PREFERENCES.maxTradeStake}, max daily loss ${DEFAULT_USER_PREFERENCES.maxDailyLoss}, max positions ${DEFAULT_USER_PREFERENCES.maxOpenPositions}, trading off, mode ${DEFAULT_USER_PREFERENCES.executionMode}.`,
  ].join("\n");
}
...
export function formatUserSettings(input: {
  ...
  const lines = [
    ...
    `${s.executionMode === "paper" ? "Mode: paper (no on-chain orders)" : "Mode: testnet"}`,
